...
router.get("/trips/:tripId/activities", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json(items.map(a => ({ ...a, description: a.description ?? null, time: a.time ?? null, location: a.location ?? null, lat: a.lat ?? null, lon: a.lon ?? null, imageUrl: a.imageUrl ?? null, locationUrl: a.locationUrl ?? null, notes: a.notes ?? null, sortOrder: a.sortOrder ?? null })));
});

router.post("/trips/:tripId/activities", requireTripParticipant(), async (req, res): Promise<void> => {
  ...
  res.status(201).json({ ...item, description: item.description ?? null, time: item.time ?? null, location: item.location ?? null, lat: item.lat ?? null, lon: item.lon ?? null, imageUrl: item.imageUrl ?? null, locationUrl: item.locationUrl ?? null, notes: item.notes ?? null, sortOrder: item.sortOrder ?? null });
});

router.patch("/trips/:tripId/activities/:activityId", requireTripParticipant(), async (req, res): Promise<void> => {
  ...
  res.json({ ...item, description: item.description ?? null, time: item.time ?? null, location: item.location ?? null, lat: item.lat ?? null, lon: item.lon ?? null, imageUrl: item.imageUrl ?? null, locationUrl: item.locationUrl ?? null, notes: item.notes ?? null, sortOrder: item.sortOrder ?? null });
});

router.post("/trips/:tripId/activities/reorder", requireTripParticipant(), async (req, res): Promise<void> => {
  const params = ListActivitiesParams.safeParse(req.params);
  if (!params.success) { res.status(400).json({ error: "Invalid tripId" }); return; }
  ...
  await Promise.all(
    (ids as number[]).map((id, index) =>
      db.update(activitiesTable)
        .set({ sortOrder: index })
        .where(and(eq(activitiesTable.id, id), eq(activitiesTable.tripId, tripId)))
    )
  );
  res.json({ success: true });
});

router.delete("/trips/:tripId/activities/:activityId", requireTripParticipant(), async (req, res): Promise<void> => {
