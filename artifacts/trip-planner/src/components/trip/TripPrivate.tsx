...
import { StickyNote, Plus, Trash2, FileText, CalendarDays } from 'lucide-react';
...
export function TripNotes({ tripId }: { tripId: number }) {
  ...
  const handleCreate = () => {
    createNote.mutate({ tripId, data: { title: 'New Note', content: '' } }, {
      onSuccess: (newNote) => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 min-h-[500px]">
      <div className="md:w-64 shrink-0 flex flex-col gap-4">
        <Button onClick={handleCreate} className="w-full shadow-sm"><Plus className="h-4 w-4 mr-2"/> New Note</Button>
        <div className="space-y-2 overflow-y-auto">
          {notes?.map(note => (
            <button 
              key={note.id}
              onClick={() => setActiveNoteId(note.id)}
              className={`w-full text-left p-3 rounded-lg border transition-all ${activeNoteId === note.id ? 'bg-primary/10 border-primary/50 text-primary' : 'bg-card hover:bg-muted/50 border-transparent hover:border-border'}`}
            >
          ...
          {notes?.length === 0 && <p className="text-muted-foreground text-sm text-center py-4">No notes yet.</p>}
        </div>
      </div>
      
      <div className="flex-1 bg-card border rounded-xl shadow-sm flex flex-col">
        {activeNote ? (
          <NoteEditor tripId={tripId} note={activeNote} onDelete={() => handleDelete(activeNote.id)} />
        ) : (
...
function NoteEditor({ tripId, note, onDelete }: { tripId: number, note: any, onDelete: () => void }) {
  ...
  }, [note.id]); // only reset on ID change, not every render

  const mutateFnRef = useRef(updateNote.mutate);
  mutateFnRef.current = updateNote.mutate;

  const saveChanges = useCallback((newTitle: string, newContent: string) => {
    mutateFnRef.current({ tripId, tripNoteId: note.id, data: { title: newTitle, content: newContent } }, {
      onSuccess: () => {
        // just patch cache so it doesn't interrupt typing
        queryClient.setQueryData(getListTripNotesQueryKey(tripId), (old: any) => {
          if (!old) return old;
          return old.map((n: any) => n.id === note.id ? { ...n, title: newTitle, content: newContent } : n);
        });
      }
    });
  ...
  }, [title, content, note.title, note.content, saveChanges]);
...
export function TripDocuments({ tripId }: { tripId: number }) {
  ...
  if (isLoading) return <div>Loading docs...</div>;
  ...
  return (
    <div className="max-w-3xl space-y-8">
      ...
      <div className="grid gap-4">
        {docs?.map(doc => (
          <div key={doc.id} className="bg-card border rounded-xl p-5 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-mono text-muted-foreground mt-0.5">{doc.number}</p>
              </div>
            </div>
        ...
        {docs?.length === 0 && <p className="text-muted-foreground text-center py-8">No documents added.</p>}
      </div>
    </div>
  );
}
